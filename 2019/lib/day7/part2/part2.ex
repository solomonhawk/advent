defmodule Day7.Part2 do
  ...
  https://elixirforum.com/t/proper-use-of-task-async/13259/8
  """
  ...
  def calculate(program) do
    base_config = Amplifier.Configuration.new(mode: :loop)
    variations = permutations(base_config.phase_settings)

    Amplifier.Aggregator.start_link(%{
      total: Enum.count(variations),
      reply_to: self()
    })

    # for each permutation in the phase_settings
    pids =
      for variation <- variations do
        name = Enum.join(variation, "")

        # start an amplifier controller supervisor
        {:ok, pid} = Amplifier.DynamicSupervisor.start_link(:ok, {:global, name})

        # controller spawns 5 workers (one for each amplifier in the loop)
        Amplifier.DynamicSupervisor.optimize(
          {:global, name},
          program,
          struct(base_config, phase_settings: variation)
        )
